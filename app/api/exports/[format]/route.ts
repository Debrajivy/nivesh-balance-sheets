...
export async function GET(req:Request,ctx:RouteContext<"/api/exports/[format]">){
 try{
  ...
  const auth=await requireCapability("export",requested);
  ...
  if(format==="xlsx"){
   ...
   for(const line of data.accepted){const row=line as Row,source=row.source as Row|undefined;sheet.addRow([Number(row.category)<=8?"Asset":"Liability",row.name,row.categoryName,row.entity,row.basis,row.costAmount||"",row.amount,row.asAtDate,row.freshnessState,source?.filename||"",row.sourceLocation])}
   sheet.addRow([]);sheet.addRow(["Total assets",data.totals.assets]);sheet.addRow(["Total liabilities",data.totals.liabilities]);sheet.addRow(["Net worth",data.totals.netWorth]);sheet.columns.forEach(column=>{column.width=18});
   ...
   write("NIVESH - TRUE AND FAIR STATEMENT",16,true);write(`Assets ${money(data.totals.assets)} | Liabilities ${money(data.totals.liabilities)} | Net worth ${money(data.totals.netWorth)}`,11,true);write("Every figure includes its source and location. Not a legally audited statement.",8);y-=8;
   for(const line of data.accepted){const row=line as Row,source=row.source as Row|undefined;write(`${Number(row.category)<=8?"ASSET":"LIABILITY"} | ${String(row.name)} | ${money(Number(row.amount))}`,9,true);write(`${String(row.entity)} | ${String(row.categoryName)} | ${String(row.basis)} | ${String(row.freshnessState)}`);write(`Source: ${String(source?.filename||"Missing")} - ${String(row.sourceLocation)}`,8);y-=3}
 ...
 }catch(error){return NextResponse.json({error:error instanceof Error?error.message:"Export failed"},{status:error instanceof Error&&error.message==="FORBIDDEN"?403:400})}
}
